...
class AbstractJSONToolParser(ToolParser):
    """
    Abstracts the parsing logic for models that output tool calls in a JSON array
    wrapped by specific start/end tokens. Subclasses must define the tokens
    and the method to extract function names and arguments from the JSON objects.
    """
    ...
    def __init__(
        ...
        self.tool_calls_prefix = tool_calls_prefix
        self.tool_calls_suffix = tool_calls_suffix
    ...
    def _extract_tool_call_data(self, tool_call_obj: dict) -> tuple[str | None, dict | None]:
        """
        Extract function name and arguments dictionary from a raw tool call object.

        Args:
            tool_call_obj: A dictionary representing a single tool call from the JSON list.

        Returns:
            A tuple containing (function_name, arguments_dict).
            Returns (None, None) if the object is malformed or incomplete.
        """
    ...
    def _parse_tool_call_objects(self, tool_call_objects: list[dict]) -> list[ToolCall]:
        ...
        for obj in tool_call_objects:
            ...
            function_name, arguments = self._extract_tool_call_data(obj)
    ...
    def _finalize_previous_tool(self) -> DeltaMessage | None:
        ...
        _, arguments = self._extract_tool_call_data(prev_tool)
    ...
    def _send_tool_name(self, current_tool_call: dict) -> DeltaMessage | None:
        ...
        function_name, _ = self._extract_tool_call_data(current_tool_call)
    ...
    def _stream_arguments(
        ...
        function_name, arguments = self._extract_tool_call_data(current_tool_call)
    ...
    def _calculate_argument_diff(
            self, function_name: str | None, args_json: str, json_str: str, sent: int
            ) -> str | None:
        """
        ...
        prev_tool = self.prev_tool_call_arr[self.current_tool_id]
        prev_function_name, prev_args = self._extract_tool_call_data(prev_tool)
